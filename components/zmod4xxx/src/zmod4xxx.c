...
zmod4xxx_err zmod4xxx_read_status(zmod4xxx_dev_t *dev, uint8_t *status)
{
    ...
    ret = dev->read(dev->i2c_dev, ZMOD4XXX_ADDR_STATUS, &st, 1);
    if (0 != ret) {
        return ERROR_I2C;
    }
    *status = st;
...
zmod4xxx_err zmod4xxx_check_error_event(zmod4xxx_dev_t *dev)
{
    ...
    ret = dev->read(dev->i2c_dev, 0xB7, &data_buf, 1);
    if (ret) {
        return ERROR_I2C;
    }
...
zmod4xxx_err zmod4xxx_read_sensor_info(zmod4xxx_dev_t *dev)
{
    ...
    do {
        i2c_ret = dev->write(dev->i2c_dev, ZMOD4XXX_ADDR_CMD, &cmd, 1);
        if (i2c_ret) {
            return ERROR_I2C;
        }
    ...
    i2c_ret =
        dev->read(dev->i2c_dev, ZMOD4XXX_ADDR_PID, data_buf, ZMOD4XXX_LEN_PID);
    if (i2c_ret) {
        return ERROR_I2C;
    }
    ...
    i2c_ret = dev->read(dev->i2c_dev, ZMOD4XXX_ADDR_CONF, dev->config,
                        ZMOD4XXX_LEN_CONF);
    if (i2c_ret) {
        return ERROR_I2C;
    }

    i2c_ret = dev->read(dev->i2c_dev, ZMOD4XXX_ADDR_PROD_DATA, dev->prod_data,
                                           ...
                                           uint8_t *track_num)
{
    ...
    ret = dev->read(dev->i2c_dev, ZMOD4XXX_ADDR_TRACKING, track_num,
...
zmod4xxx_err zmod4xxx_init_sensor(zmod4xxx_dev_t *dev)
{
    ...
    i2c_ret = dev->read(dev->i2c_dev, 0xB7, data_r, 1);
    if (i2c_ret) {
        return ERROR_I2C;
    }
    ...
    i2c_ret = dev->write(dev->i2c_dev, dev->init_conf->h.addr, hsp,
                         dev->init_conf->h.len);
    if (i2c_ret) {
        return ERROR_I2C;
    }
    i2c_ret = dev->write(dev->i2c_dev, dev->init_conf->d.addr,
                         dev->init_conf->d.data_buf, dev->init_conf->d.len);
    if (i2c_ret) {
        return ERROR_I2C;
    }
    i2c_ret = dev->write(dev->i2c_dev, dev->init_conf->m.addr,
                         dev->init_conf->m.data_buf, dev->init_conf->m.len);
    if (i2c_ret) {
        return ERROR_I2C;
    }
    i2c_ret = dev->write(dev->i2c_dev, dev->init_conf->s.addr,
    ...
    i2c_ret =
        dev->write(dev->i2c_dev, ZMOD4XXX_ADDR_CMD, &dev->init_conf->start, 1);
    if (i2c_ret) {
        return ERROR_I2C;
    }
    do {
    ...
    i2c_ret = dev->read(dev->i2c_dev, dev->init_conf->r.addr, data_r,
                        dev->init_conf->r.len);
    if (i2c_ret) {
        return ERROR_I2C;
    }
...
zmod4xxx_err zmod4xxx_init_measurement(zmod4xxx_dev_t *dev)
{
    ...
    i2c_ret = dev->write(dev->i2c_dev, dev->meas_conf->h.addr, hsp,
                         dev->meas_conf->h.len);
    if (i2c_ret) {
        return ERROR_I2C;
    }
    i2c_ret = dev->write(dev->i2c_dev, dev->meas_conf->d.addr,
                         dev->meas_conf->d.data_buf, dev->meas_conf->d.len);
    if (i2c_ret) {
        return ERROR_I2C;
    }
    i2c_ret = dev->write(dev->i2c_dev, dev->meas_conf->m.addr,
                         dev->meas_conf->m.data_buf, dev->meas_conf->m.len);
    if (i2c_ret) {
        return ERROR_I2C;
    }
    i2c_ret = dev->write(dev->i2c_dev, dev->meas_conf->s.addr,
...
zmod4xxx_err zmod4xxx_start_measurement(zmod4xxx_dev_t *dev)
{
    ...
    ret =
        dev->write(dev->i2c_dev, ZMOD4XXX_ADDR_CMD, &dev->meas_conf->start, 1);
    if (ret) {
        return ERROR_I2C;
    }
    return ZMOD4XXX_OK;
...
zmod4xxx_err zmod4xxx_read_adc_result(zmod4xxx_dev_t *dev, uint8_t *adc_result)
{
    ...
    ret = dev->read(dev->i2c_dev, dev->meas_conf->r.addr, adc_result,
                    dev->meas_conf->r.len);
    if (ret) {
        return ERROR_I2C;
    }
